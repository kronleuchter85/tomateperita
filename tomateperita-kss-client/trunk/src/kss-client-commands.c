...
#include "kss-client-commands.h"
	...
	Bool isCommandDisconnect(char * cmd){
		return equalsStrings(SHELL_COMMAND_DISCONNECT , trim(cmd) );
	}
	Bool isCommandQuit(char * cmd){
		return equalsStrings(SHELL_COMMAND_QUIT , trim(cmd) );
	}
	Bool isCommandShowDevices(char *cmd){
		return equalsStrings(SHELL_COMMAND_SHOW_DEVICES , trim(cmd) );
	}
