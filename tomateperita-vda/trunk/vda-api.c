...
int putSectorOnDisk (LogicalSector * ls) {
	...
	if(isInfoEnabled()) {
		sprintf_s(logBuf, BUFFSIZE,"[PUT] Posicion cabezal: %d, Tiempo consumido (ms): %.2f, Sectores leidos: %s", headPos, search_getTime(&search), search_getHistory(&search));
		info(logBuf); 
		}
...
LogicalSector * getSectorFromCacheOrDisk1 (tSectorId sid) {
	...
	if (isVdaAvailableCache())
			if ((ret = cache_Search (getVdaCache(), ls, sid)) == 1) 
					if(isInfoEnabled()) {
						sprintf_s(logBuf, BUFFSIZE,"[GET] Tiempo consumido (ms): %.2f, Sectores leidos: %d (en cache), Cache Hit/Miss %d/%d", cache_getAccessTime(), sid, cache_getHitAverage(getVdaCache()), cache_getMissAverage(getVdaCache()));
						info(logBuf); 
					}
	if (ret != 1) {
				...
				if(isInfoEnabled()) {
					if (isVdaAvailableCache())
						sprintf_s(logBuf, BUFFSIZE,"[GET] Posicion cabezal: %d, Tiempo consumido (ms): %.2f, Sectores leidos: %s, Cache Hit/Miss %d/%d", headPos, search_getTime(&search), search_getHistory(&search), cache_getHitAverage(getVdaCache()), cache_getMissAverage(getVdaCache()));
					else
						sprintf_s(logBuf, BUFFSIZE,"[GET] Posicion cabezal: %d, Tiempo consumido (ms): %.2f, Sectores leidos: %s", headPos, search_getTime(&search), search_getHistory(&search));
						info(logBuf); 
					}
	}
	searchParmDestroy (&search);
...
void showInfo(void) {
	char logBuf[BUFFSIZE];
	if (isInfoEnabled()) {
		sprintf_s(logBuf, BUFFSIZE, "[SHOW] Tiempo medio de busqueda (ms): %.2f ", disk_averageTime(getDiskParms()));
		info(logBuf); 
		sprintf_s(logBuf, BUFFSIZE, "[SHOW] Tiempo peor de busqueda (ms): %.2f ", disk_worstTime(getDiskParms()));
		info(logBuf);
		info(concat(getGlobalHeap(), 2,"[SHOW] Tasa de transferencia (bytes/seg): ", itoa_buf(disk_transferRate(getDiskParms()))));
		if (isVdaAvailableCache()) {
			sprintf_s(logBuf, BUFFSIZE, "[SHOW] Tiempo medio de acceso a la cache (ms): %.2f", cache_getAccessTime());
			info(logBuf);
		}
...
void demoPutSectores(void){
	...
	int c1 = (int) strlen(contenido1);
	int c2 = (int) strlen(contenido2);
