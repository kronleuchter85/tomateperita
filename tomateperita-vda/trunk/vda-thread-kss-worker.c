...
#include "vda-state.h"
	...
	unsigned __stdcall doKssService( void* pArguments ){
		...
		do{
			...
			if(request == NULL){
				setContinueKssThread(FALSE);
				break;
			}

			if( equalsStrings(request->operationName,VDA_SECTORS_PUTTING)){
					request = receiveMpsBlockMessage(getKssConnection() , validator , getGlobalHeap());
			}
	...
	void executeOperation(MpsMessage * req , ListenSocket ls , RuntimeErrorValidator * v){
		...
		}else if(startsWith(req->operationName , VDA_SECTORS_PUTTING)){
