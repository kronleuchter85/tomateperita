...
#include "vda-api.h"
	...
	void initializeDisk();
	void processShell();
	void initializeKssConnection( HeapHandler );
	...
	void configure(){
		...
		if (!isVdaOfflineMode()) {
			initializeWinsock();
			initializeKssConnection( getGlobalHeap() );
		}
	...
	void initializeKssConnection(HeapHandler h){
		
		ListenSocket kssConnectionSocket;
		MpsHandshake * handshake;
		RuntimeErrorValidator * v = buildErrorSuccessValidator(h);
		
		if( isInfoEnabled() ) info( "Iniciando la conexion con el KSS");
		
		kssConnectionSocket = openConnectionSocket( getKssAddress() , getKssPort() );

		handshake = buildMpsHandhake( generateRandomKey(16,h) , VDA_HANDSHAKE , h );
		handshake = sendAndReceiveMpsHandshake( handshake , kssConnectionSocket , v , getGlobalHeap() );
		
		if( handshake != NULL && handshake->statusCode == MPS_RESPONSE_STATUS_CODE_SUCCESS ){
			info( concat( h , 2 , 
				"Handshake satisfactorio con DescriptorId: " ,
				handshake->descriptorId ) );
			setKssConnection( kssConnectionSocket );
		} else {
			setError( v , "Hubo un problema y no se realizo el handshake.");
			return;
		}
