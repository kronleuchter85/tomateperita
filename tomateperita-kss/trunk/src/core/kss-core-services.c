...
#include "kss-tdd.h"
	...
	void mountVda(char * vdaName , RuntimeErrorValidator * validator){
		...
		if(vda != NULL){
			...
			Bool formated = callFssOperationHasVdaFormat(vda->vdaName , validator);
			...
			if(formated){
				...
				setError(validator , "El dispositivo que se quiere montar requiere ser formateado");
				return ;
			}
	...
	void disconnectVda(char * vdaName , RuntimeErrorValidator * validator){
		VdaComponent * vda = getConnectedVda(vdaName);
		if(vda != NULL){
			removeConnectedVda(vda);
		}else{
	...
	int openFileInTdd(char * vdaName , char * fileName , char * openingMode , RuntimeErrorValidator * validator){
		...
		if(isOpeningModeRead(openingMode)){
			...
			tddRecord->fileSize = fileInfo->size;
