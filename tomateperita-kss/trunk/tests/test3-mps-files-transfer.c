...
#include "kss-vda-endpoint.h"
	...
	//char * imagePath = "/home/guest/images/anonimo.jpg";
//	char * destPath = "/home/guest/images/imagen-restaurada-%d.jpg";
	char * destPath = "/home/gonzalo/tomateperita-data/imagen-restaurada-%d.jpg";
	int counter = 0;
	...
	void integrateVdasWIthBlocks(){
		...
		int deAcumulador = 154588;
		int s1 = 1;
		int s2 = 2;
		int written1 , written2;

		//sprintf(dest , "/home/gonzalo/tomateperita-data/imagen-restaurada-%d.jpg" , counter);
		sprintf(dest , destPath , counter);
		file1 = fopen(dest , "w");
		...
		while(sector1 != s1 && sector2 != s2){
			...
			if (deAcumulador < 512 ){
				written1 = fwrite( block->sector1.data, sizeof(char) , deAcumulador , file1);
			} else {
				written1 = fwrite( block->sector1.data, sizeof(char) , block->sector1.dataLength , file1);
			}

			deAcumulador -= written1;

			if (deAcumulador < 512 ){
				written2 = fwrite( block->sector2.data, sizeof(char) , deAcumulador , file1);
			} else {
				written2 = fwrite( block->sector2.data, sizeof(char) , block->sector2.dataLength , file1);
			}

			deAcumulador -= written2;
