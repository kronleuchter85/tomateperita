...
#include "ftp.h"
	...
	void executeCommandHelp(ClientThreadResource * );
	void executeCommandDele(ClientThreadResource * res , char *);
	...
	unsigned __stdcall serviceControlThread( void* pArguments ){
		...
		sendBytes(ctr->controlSocket , buildResponseMessageType220(ctr->heapHandler) , 
			(int) strlen(buildResponseMessageType220(ctr->heapHandler)));
		...
		if(isDebugEnabled())
			debug( concat(ctr->heapHandler , 2 , "Cerrando socket remoto SocketId=" , 
				intToString((int) ctr->controlSocket , ctr->heapHandler)) );	
	...
	void execute(char * aRequest , ClientThreadResource * aResource ){
		...
		}else if(startsWith(aRequest , COMMAND_HELP)){

			executeCommandHelp(aResource);

		}else if(startsWith(aRequest , COMMAND_DELETE)){

			executeCommandDele(aResource, aRequest );
	...
	void executeCommandPasv(ClientThreadResource * res ){		
		...
		sendBytes(res->controlSocket , buildResponseMessageType227(res->heapHandler , port) , 
			(int) strlen(buildResponseMessageType227(res->heapHandler , port)));
	}
	...
	void executeCommandStore(ClientThreadResource * res , char * request ){
		...
		if(isOfflineStartMode()){
			...
			res->argument = concat(res->heapHandler , 2, res->relativeWorkingDir , fileName);
		}		
		
		res->transferTask = STOR_TRANSFER;
		sendBytes(res->controlSocket , buildResponseMessageType150(res->heapHandler) , 
			(int) strlen(buildResponseMessageType150(res->heapHandler)));
		doDataService(res);
		blockMutex( res->dataThreadHandler );
		sendBytes(res->controlSocket , res->resultMessage , (int) strlen(res->resultMessage));
	}
	...
	void executeCommandRetrieve(ClientThreadResource * res , char * request ){		
		...
		if(isOfflineStartMode()){
			...
			res->argument = concat(res->heapHandler , 2 , res->relativeWorkingDir , path);
		}
	...
	void executeCommandUser(ClientThreadResource * res ){		
		sendBytes(res->controlSocket ,  buildResponseMessageType230(res->heapHandler), 
			(int) strlen( buildResponseMessageType230(res->heapHandler)));
	}
	...
	void executeCommandNotAllowed(ClientThreadResource * res ){		
		sendBytes(res->controlSocket , buildResponseMessageType500(res->heapHandler) , 
			(int) strlen(buildResponseMessageType500(res->heapHandler)));
	}
	...
	void executeCommandType(ClientThreadResource * res , char * type ){	
		sendBytes(res->controlSocket , buildResponseMessageType200(res->heapHandler) , 
			(int) strlen(buildResponseMessageType200(res->heapHandler)));
	}
	...
	void executeCommandCwd(ClientThreadResource * res , char * path ){	
		...
		sendBytes(res->controlSocket , buildResponseMessageType250( res->heapHandler) , 
			(int) strlen(buildResponseMessageType250( res->heapHandler)));
	}
	...
	void executeCommandHelp(ClientThreadResource * res ){
		res->transferTask = HELP_TRANSFER;
		sendBytes(res->controlSocket ,  buildResponseMessageType150(res->heapHandler) , 
			(int) strlen( buildResponseMessageType150(res->heapHandler)));
		doDataService(res);	
		blockMutex(res->dataThreadHandler);	
	...
	void executeCommandDele(ClientThreadResource * res, char * request){
		char * path = trimAll(replaceAll(request , COMMAND_DELETE , "") , res->heapHandler);	
