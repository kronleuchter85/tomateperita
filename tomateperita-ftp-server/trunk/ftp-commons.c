...
#include "ftp-commons.h"
	...
	char * decodePort(char * m , HeapHandler h){
		...
		encodedPort = subString(m , i , (int) strlen(m) , h);

		q = subString(encodedPort , 0 , indexOfString(encodedPort , ",")-1 , h);
		r = subString(encodedPort , indexOfString(encodedPort , ",")+1 , (int) strlen(encodedPort) , h);
	...
	char * parseHostAndPort(char * request , HeapHandler h){
		return subString(request , 5 , (int) strlen(request) , h);
	}
