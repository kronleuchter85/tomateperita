...
#include <time.h>
	...
	int patternsFoundInBuffer(char *buf, int sizeBuf, char*pattern, int sizePat){
		int founds;
		char *p, *finbuf;

		if ((buf == NULL) || (pattern == NULL) || (sizePat > sizeBuf)) return 0;
		founds = 0;
		finbuf = buf + sizeBuf - sizePat;
		p = buf;
		while (p < finbuf) 
			if ((*p == *pattern ) && (memcmp(p, pattern, sizePat)==0)) {
				founds++;
				p+=sizePat;
			}
			else p++;
		if ((p == finbuf) && (memcmp(p, pattern, sizePat)==0)) founds++;
		return founds;
	}

	/* Devuelve el tamaño del nuevo buffer */
	char* replaceAll(char *buf , int * sizeBuf, char *oldPattern, int sizeOldPat, char *newPattern, int sizeNewPat, HeapHandler hh){
		char *p, *b, *bd, *bf;
		char *newbuf;
		int size, count, salir;

		count = patternsFoundInBuffer(buf, *sizeBuf, oldPattern, sizeOldPat);
		if (count == 0) 
			return buf;

		size = *sizeBuf + (sizeNewPat - sizeOldPat) * count * sizeof (char);
		newbuf = (char *)allocateMemory(hh, size);
		if (newbuf == NULL) return NULL;

		b=buf;
		bd=buf + *sizeBuf - sizeOldPat; //Hasta donde es necesario recorrer buf para comparar
		bf=buf + *sizeBuf - 1; //Ultima posicion de buf
		p=newbuf;
		salir=0;
		while (salir==0){
			if ((b<=bd) && (memcmp(b, oldPattern, sizeOldPat)==0)){
				memcpy(p, newPattern, sizeNewPat);
				if(b<bd) {
					p+=sizeNewPat;
					b+=sizeOldPat;
				}
		...
		printf("%d-%d\n",size,p-newbuf);
		*sizeBuf = size; //Reescribo este valor para saber cuanto mide el nuevo buffer
		return newbuf;
	}
