...
#include "windows-commons-mps-encoder.h"
	...
	List getMpsArguments(char * payload , int argsCount, RuntimeErrorValidator * v , HeapHandler hh){
		...
		if(argsCount > 0){
			...
			if(argsCount == 1){
				char * value = strdup(searchingString) ;

				value = decode(value);

				addNode(args , value , hh);
			} else if(argsCount == 2){
				int indexSeparation = indexOfChar(searchingString , ' ');

				char * first = subString(searchingString , 0 , indexSeparation - 1 , hh);
				char * second = strdup(searchingString + indexSeparation + 1);
	
				first = decode(first);
				second = decode(second);

				addNode(args , first , hh);
				addNode(args , second , hh);
			}else if (argsCount == 4){
				int indexSeparation1 = indexOfChar(searchingString , ' ');
				int indexSeparation2 = indexOfChar(searchingString + indexSeparation1+1, ' ');
				int indexSeparation3 = indexOfChar(searchingString + indexSeparation1 + 1 + indexSeparation2 + 1 , ' ');
				char * first = subString(searchingString , 0 , indexSeparation1 - 1, hh);
				char * second = subString(searchingString + indexSeparation1 + 1 , 0 , indexSeparation2 - 1, hh);
				char * thirt = subString(searchingString + indexSeparation1 + 1 + indexSeparation2 + 1 , 0 , indexSeparation3 - 1, hh);
				char * fourth = strdup(searchingString + indexSeparation1 + 1 + indexSeparation2 + 1 + indexSeparation3 + 1, hh);
				...
				addNode(args , first, hh);
				addNode(args , second, hh);
				addNode(args , thirt, hh);
				addNode(args , fourth, hh);
